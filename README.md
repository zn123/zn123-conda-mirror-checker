...
- 每个镜像同时做三件事：
  1. **先拉 `current_repodata.json` 验证索引**（conda 默认首选、约 1/7 体积、最快）；若它 404 / 空 / 非 JSON，则**自动回退到完整 `repodata.json`**（权威索引），对齐 conda 自身的 `repodata_fns` 行为。结果会标注实际用的是哪个文件，避免「镜像只有 repodata.json 却被判 fail」的假阴性。
...
```json
{
  ...
  "repodataFile": "current_repodata.json",
...
- `repodataFile`：本次实际探测所用的索引文件——`current_repodata.json`（默认首选）或 `repodata.json`（前者不可用时的回退）。前端索引单元格会标注，便于判断镜像是否缺 `current_repodata.json`。
...
- `pythonLatest`：从本次实际使用的索引文件（`current_repodata.json`，回退时为 `repodata.json`）解析出的 python **最新版本号**（如 `3.14.7`）；解析失败为 `null`。
...
- **`current_repodata.json` 优先、`repodata.json` 回退**：conda 4.7+ 客户端默认 `repodata_fns = ('current_repodata.json', 'repodata.json')`——先拉体积约 1/7 的 `current_repodata.json`（最快路径），仅当 404 / 空 / 非 JSON 时才回退到权威完整 `repodata.json`。工具复刻此行为：只探 `current` 会漏掉「仅有 `repodata.json` 的旧/部分镜像」造成假阴性，回退后结果用 `repodataFile` 字段标明实际用了哪个；回退用独立临时文件避免与首次下载共享文件句柄导致竞态。
